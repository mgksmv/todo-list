...
За основу фронтенд части взял официальный starter kit от Laravel, но избавился от Inertia и разделил проекта на 2 отдельных проекта - `backend` (Laravel) и `frontend` (Nuxt).
...
Окружение:<br />
`dev` - разработка<br />
`prod-http` - продакшн без домена (запуск с IP адресом)<br />
`prod` - продакшн<br />
